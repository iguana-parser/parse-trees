...
private class ToScalaCode extends Visitor[SPPFNode] with Id {
  ...
  def visit(node: SPPFNode): VisitResult[T] = node match {
    ...
    case n:NonterminalNode =>
      ...
      sb ++= s"""val node${getId(node)} = NonterminalNode(registry.getSlot("$name"), registry.getSlot("${children.head.slot.toString}"), node${getId(children.head.leftChild)}, input);\n"""
    ...
    case IntermediateNode(name, leftExtent, rightExtent, children) =>
      children.foreach(p => p.children.foreach(visit(_)))
      sb ++= s"""val node${getId(node)} = IntermediateNode(registry.getSlot("$name"), node${getId(children.head.leftChild)}, node${getId(children.head.rightChild)});\n"""
    ...
    case TerminalNode(name, leftExtent, rightExtent, input) =>
      sb ++= s"""val node${getId(node)} = TerminalNode(registry.getSlot("$name"), $leftExtent, $rightExtent, input);\n"""
      None
  }
}